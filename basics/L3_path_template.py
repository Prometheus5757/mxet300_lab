...
time_split = d1/forward_velocity
...
motions = [
    [forward_velocity, 0.0, time_split],            # Motion 1
    [0.0, (pi/2)/time_split, time_split*1.2],       # Motion 2
    [forward_velocity, 0.0, time_split],            # Motion 3
    [0.0, (pi/2)/time_split, time_split],           # Motion 4
    [forward_velocity, 0.0, time_split],            # Motion 5
    [0.0, (-pi/2)/time_split, time_split],          # Motion 6
    [forward_velocity, 0.0, time_split],            # Motion 7
    [0.0, (-pi/2)/time_split, time_split],          # Motion 8
    [forward_velocity, 0.0, time_split],            # Motion 9
]
...
for  count, motion in enumerate(motions):
    ...
    myBatt = voltRead.readVolts()   
    log.tmpFile(motion[0],"for_vel.txt") #Node RED forward velocity
    log.tmpFile(motion[1],"ang_vel.txt") #Node RED Angular Velocity
    log.tmpFile(myBatt,"batt_volt.txt") # Node RED battery voltage
